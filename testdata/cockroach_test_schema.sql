...
CREATE TABLE magic (
  ...
  strange_three timestamp without time zone default (now()),
  strange_four  timestamp with time zone default (now()),
...
CREATE INDEX ON pilot_languages (pilot_id);
CREATE INDEX ON pilot_languages (language_id);
