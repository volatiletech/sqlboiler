...
func test{{$tableNamePlural}}Upsert(t *testing.T) {
	...
	defer tx.Rollback()
	if err = {{$varNameSingular}}.Upsert(tx, {{if (or (eq .DriverName "psql") (eq .DriverName "crdb"))}}false, nil, {{end}}nil); err != nil {
	...
	if err = {{$varNameSingular}}.Upsert(tx, {{if (or (eq .DriverName "psql") (eq .DriverName "crdb"))}}true, nil, {{end}}nil); err != nil {
		t.Errorf("Unable to upsert {{$tableNameSingular}}: %s", err)
	}
