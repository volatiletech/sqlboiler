...
func (p *pgTester) setup() error {
  ...
  p.dbName = viper.GetString("crdb.dbname")
  p.host = viper.GetString("crdb.host")
  p.user = viper.GetString("crdb.user")
  p.pass = viper.GetString("crdb.pass")
  p.port = viper.GetInt("crdb.port")
  p.sslmode = viper.GetString("crdb.sslmode")
...
func (p *pgTester) conn() (*sql.DB, error) {
  ...
  var err error
  p.dbConn, err = sql.Open("postgres", driver.CockroachDBBuildQueryString(p.user, p.pass, p.testDBName, p.host, p.port, p.sslmode))
  if err != nil {
    return nil, err
  }
