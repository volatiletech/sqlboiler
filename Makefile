# Everything in this make file assumes you have the ability to run the commands
# as they are written (meaning .my.cnf and .pgpass files set up with admin
# users) as well as the mssql db set up with the sa / Sqlboiler@1234
# credentials. See testdata/env.sh. For CockroachDB 'root' is used for
# everything until they decide what to do with user authentication and insecure
# clusters:
# - https://github.com/cockroachdb/cockroach/pull/22328
# - https://github.com/cockroachdb/cockroach/issues/24061
# - https://github.com/cockroachdb/cockroach/issues/16188
...
testdata/env.mk: testdata/env.sh
	sed 's/"//g ; s/=/:=/ ; s/export //g' < $< > $@

# Builds all software and runs model tests
.PHONY: tests
tests: test-psql test-mysql test-mssql test-crdb

# Creates super users
.PHONY: test-users
test-users: test-user-psql test-user-mysql test-user-mssql test-user-crdb

# Creates databases
.PHONY: test-dbs
test-dbs: test-db-psql test-db-mysql test-db-mssql test-db-crdb

# Runs tests on the drivers
.PHONY: driver-tests
driver-tests: driver-test-psql driver-test-mysql driver-test-mssql driver-test-crdb

# Creates regular databases
.PHONY: driver-dbs
driver-dbs: driver-db-psql driver-db-mysql driver-db-mssql driver-db-crdb

# Creates regular users with access to only one DB, may modify the DB
# to ensure write access to the user
.PHONY: driver-users
driver-users: driver-user-psql driver-user-mysql driver-user-mssql driver-user-crdb
...
.PHONY: test-db-crdb
test-db-crdb:
	cockroach sql --insecure --user root --execute "drop database if exists $(SQLBOILER_TEST_DB);"
	cockroach sql --insecure --user root --execute "create database $(SQLBOILER_TEST_DB);"
	cockroach sql --insecure --user root --database $(SQLBOILER_TEST_DB) < testdata/crdb_test_schema.sql
...
.PHONY: test-generate-crdb
test-generate-crdb:
	printf "[crdb]\nhost=\"localhost\"\nport=26257\nuser=\"%s\"\npass=\"%s\"\ndbname=\"%s\"\nsslmode=\"disable\"\n" "root" "" $(SQLBOILER_TEST_DB) > sqlboiler.toml
	./sqlboiler --wipe crdb 
...
.PHONY: driver-db-crdb
driver-db-crdb:
	cockroach sql --insecure --user root --execute "create database $(SQLBOILER_TEST_DRIVER_DB);"
...
.PHONY: driver-test-crdb
driver-test-crdb:
	DRIVER_DB=$(SQLBOILER_TEST_DRIVER_DB) DRIVER_USER="root" DRIVER_PASS="" go test -v -race github.com/volatiletech/sqlboiler/drivers/sqlboiler-crdb/driver
