...
func (o *{{$alias.UpSingular}}) InsertWithSchema(schema string, {{if .NoContext}}exec boil.Executor{{else}}ctx context.Context, exec boil.ContextExecutor{{end}}, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO %s ({{.LQ}}%s{{.RQ}}) %%sVALUES (%s)%%s", schemaTable, strings.Join(wl, "{{.RQ}},{{.LQ}}"), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			{{if .Dialect.UseDefaultKeyword -}}
			cache.query = fmt.Sprintf("INSERT INTO %s %%sDEFAULT VALUES%%s", schemaTable)
			{{else -}}
			cache.query = fmt.Sprintf("INSERT INTO %s () VALUES ()%%s%%s", schemaTable)
			{{end -}}
		}
